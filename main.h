...
class tableBody {
public:
	tableBody(std::ostream &err)noexcept :err(err) { }
	~tableBody() noexcept = default;
	std::ostream &err;
	auto cbegin()const noexcept { return bodies.cbegin(); }
	auto cend()const noexcept { return bodies.cend(); }
	template<typename... T>auto new_body(T&&... t)noexcept {
		bodies.emplace_back(std::move(t)...);
...
template<>
class table<Mode::system> :protected tableBody<std::string> {
	using super = tableBody<std::string>;
	//约束表 constraints map
	std::vector<constraint> constraints;
public:
	table(std::ostream &err)noexcept :super(err) { }
	...
	void new_constraint(
		...
		try {
			...
			if (a != "base") {
				auto ia = std::find_if(super::cbegin(), super::cend(), [&a](const std::string &_body)->bool { return _body == a; });
				if (ia == super::cend()) {
					constexpr char tmp = '1';
					_a = new_body(a.c_str());
					err << "A body named " << a << " has been emplaced autimatically." << std::endl;
				}
				else _a = ia - super::cbegin();
			}
			if (b != "base") {
				auto ib = std::find_if(super::cbegin(), super::cend(), [&b](const std::string &_body)->bool { return _body == b; });
				if (ib == super::cend()) {
					constexpr char tmp = '1';
					_b = new_body(b.c_str());
					err << "A body named " << b << " has been emplaced autimatically." << std::endl;
				}
				else _b = ib - super::cbegin();
			}
	...
	void solve(std::ostream &out) noexcept {
		try {
			Math::NormalMatrix<constant> constr(super::size() * 3, constraints.size());
			...
			auto rank = constr.to_diagon(super::size() * 3, constraints.size());
			out << "After solving:" << std::endl << std::setw(5) << constr << std::endl;
			if (rank < super::size() * 3) {
				out
					<< "Mutable system! It is "
					<< (constr.no_more_than_one_nonzero_each_line() ? "variable" : "transient")
					<< " system. And it has "
					<< super::size() * 3 - rank << " free degree. " << std::endl;
			}
			else if (super::size() * 3 == constraints.size()) {
				out << "Statically determinate structure!" << std::endl;
			}
			else {
				out << "Statically indeterminate structure! Its degree of statical indeterminacy is " << constraints.size() - super::size() * 3 << '.' << std::endl;
			}
		}
		catch (const char *e) {
			err
...
public:
	...
	holder x, y;
	constant begin, end;
	body(std::string name, holder &&x, holder &&y, const char *begin, const char *end)noexcept
...
template<>
class table<Mode::structure> :protected tableBody<body> {
	using super = tableBody<body>;
	class external {
	public:
		constant t;
		constant fx, fy;
...
public:
	...
	key new_body(const char *name, const char *x, const char *y, const char *begin, const char *end) {
		...
		return super::new_body(name, std::move(fun_x), std::move(fun_y), begin, end);
	}
	...
	void new_constraint(
		...
		try {
			key _a = key(-1), _b = key(-1);
			if (a != "base") {
				auto ia = std::find_if(super::cbegin(), super::cend(), [&a](const body &_body)->bool { return _body == a; });
				if (ia == super::cend()) {
			...
			if (b != "base") {
				auto ib = std::find_if(super::cbegin(), super::cend(), [&b](const body &_body)->bool { return _body == b; });
				if (ib == super::cend()) {
	...
	void solve(std::ostream &out) noexcept {
		try {
			Math::NormalMatrix<constant> constr(super::size() * 3, constraints.size() + 1);
					...
					if (j >= constraints.size()) {
						...
						for (auto ext = exts.first; ext != exts.second; ++ext) {
							...
							case 1:
								res += (*ext).second.fy;
							case 2:
								res += this->super::operator[](i / 3).x->getValue((*ext).second.t) * (*ext).second.fy - this->super::operator[](i / 3).y->getValue((*ext).second.t) * (*ext).second.fx;
							default:
								break;
							}
						}
